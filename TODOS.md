...
## Phase 2: LangChain and RAG
 ...
 - [ ] Sandbox: After 10 questions have the chatbot create a Google Books search engine query
    - [x] LangGraph has a chat_node for generating a response to user and a search node for generating book search query every time the model is invoked. 
    - [x] Add another node to generate a list of 12 books as (title, author) to recommend
    - [x] After 10 questions the LLM just lists it's final 12 book title and author. See if they are mostly real books.
    - [ ] Write a function that uses the Google Books API to just look up a book title and author
    - [ ] Optional: make sure the recommendation node is not "invoked"/"ran" every time.
